...
* ATTINY85 must be configured to 8MHz clock effective
*
*--------------------------------------------------------------------------*/
...
int8_t  dht_read(uint8_t *temperature, uint8_t *humidity)
{
    ...
    // send start sequence 1 LOW pulse 
    DHT_PIN_OUTPUT();
    DHT_PIN_LOW();
    ...
    for (i = 0; i < 5; ++i) {
        for (j = 0; j < 8; ++j) 
         ...
         if (dht_await_state(0) > 20)  data[i] |= 1;   
          };
    };
...
// used for 4 seconds of sleeping and to conserve energy
//
ISR(WDT_vect) {
