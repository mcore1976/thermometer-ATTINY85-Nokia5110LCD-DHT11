/*--------------------------------------------------------------------------*
* Temperature and Humidity measure from DHT22 connected to PB3
...
* ATTINY85 has factory fusebits (lfuse=62hex) - 8 MHz clock with DIV to 1MHz
* --------------------------------------------------------------------------*/
...
// used for 4 secs of sleeping to conserve energy
//
ISR(WDT_vect) {
...
int main(void)
{
          ...
          uint8_t firstdig, seconddig, thirddig, belowzero;
	...
	cli();
      ...
      // if so - temperature is below zero Celsius Degrees
       if (temperature_hi > 127)  
           ...
           {
             // do not change temperature reading
             belowzero = 0;
            };
          ...
          if ((temperature != lasttemperature) || (humidity != lasthumidity))
            {
              ...
              my_const_str = "  TEMPERATURE"; 
              lcdPrint(0, 0, my_const_str, 0);

              // calculate 3 digits for temperature
              firstdig = temperature / 100;
              temporary = temperature % 100; 
              seconddig = temporary / 10;
              thirddig = temporary % 10; 


              // display minus sign, temperature digits and Celsius sign
              if (belowzero == 1)  lcdPrintBChar(1, 0, 12, 1, 0);     // minus sign on first column if needed
